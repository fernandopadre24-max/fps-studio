...
require('fs').readFileSync('/app/applet/app.js', 'utf8')
    .split('\n')
    .filter(line => !line.includes('document.getElementById') && !line.includes('window.') && !line.includes('var '))
    .join('\n');
...
`;
require('fs').writeFileSync('test_val.js', code);
require('child_process').execSync('node test_val.js', {stdio: 'inherit'});
